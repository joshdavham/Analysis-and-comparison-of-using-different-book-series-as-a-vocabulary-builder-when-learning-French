...
def get_T(sentence, known_words_series):
    ...
    for word in tokens:
        ...
        rslt = known_words_series.loc[known_words_series['word'] == word.text, 'known']
...
def one_T_simulation(books_list, book_dfs, freq_list, freq_list_series, vocab_size_init=5000):
    ...
    learned_words_data = pd.DataFrame(columns=['Book', 'Chapter', 'Num_Words', 'Num_Unknown', 'Vocab_Before', 'Vocab_After'])
    ...
    for i in range(len(book_dfs)):
        ...
        for index in range(len(book_df)):
            ...
            new_data = pd.DataFrame({'Book':[i+1], 'Chapter':[index+1], 'Num_Words':[num_words_chapter], 'Num_Unknown':[num_unknown_words_chapter], 'Vocab_Before':[vocab_before], 'Vocab_After':[vocab_after]})
